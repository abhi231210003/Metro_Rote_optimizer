...
import java.io.*;
	public class GraphCode
	{
		...
		static HashMap<String, Vertex> vtcs;
		public GraphCode(){
			vtcs = new HashMap<>();
		}
		public int numVetex(){
		...
		public void addVertex(String vtname){
			Vertex vtx = new Vertex();
			vtcs.put(vtname, vtx);
		}
		public void removeVertex(String vtname) 
		{
			Vertex vtx = vtcs.get(vtname);
			ArrayList<String> keys = new ArrayList<>(vtx.nbrs.keySet());
			for (String key : keys) 
			{
				Vertex nbrVtx = vtcs.get(key);
				nbrVtx.nbrs.remove(vtname);
			}
			vtcs.remove(vtname);
		}
		public int numEdges() 
		...
		public int dijkstra(String src, String des, boolean nan) 
		{
			...
			HeapCode<DijkstraPair> heap = new HeapCode<>();
			...
			while (!heap.isEmpty()) 
			{
				...
				Vertex v = vtcs.get(rp.vname);
				for (String nbr : v.nbrs.keySet()) 
				{
					if (map.containsKey(nbr)) 
					{
						int oc = map.get(nbr).cost;
						Vertex k = vtcs.get(rp.vname);
						int nc;
						if(nan)
		...
		public ArrayList<String> get_Interchanges(String str) {
			...
			for (int i = 1; i < res.length - 1; i++){
				...
				String currentLineCode = res[i].substring(index + 1);
				if (currentLineCode.length() == 2) {
					String prevLineCode = res[i - 1].substring(res[i - 1].indexOf('~') + 1);
					String nextLineCode = res[i + 1].substring(res[i + 1].indexOf('~') + 1);
					if (prevLineCode.equals(currentLineCode)) {
						arr.add(res[i]); 
					} else {
						arr.add(res[i]); 
						count++; 
					}
					if (!currentLineCode.equals(nextLineCode)) {
		...
		public static void Create_Metro_Map(GraphCode g)
		{
		...
		public static void main(String[] args) throws IOException
		{
			...
			while(true){
				...
				System.out.println("3. GET ALL DETAILS OF TRAVELLING FROM ONE STATION TO ANOTHER");
				...
				case 3:
					...
					if(!g.containsVertex(st1) || !g.containsVertex(st2) || !g.hasPath(st1, st2, processed)){
						...
						ArrayList<String> str = g.get_Interchanges(g.Get_Minimum_Distance(st1, st2));
						int len = str.size();
						System.out.println("SHORTEST PATH HAS " + str.get(len-2)+" INTERCHANGES");
						System.out.println("~~~~~~~~~~~~~");
						...
						//shortest time
						System.out.println("\nSHORTEST TIME :: "+g.dijkstra(st1, st2, true)/60+" MINUTES");

						//Shortest path (Time Wise)
						ArrayList<String> str1 = g.get_Interchanges(g.Get_Minimum_Time(st1, st2));
						int length = str1.size();
						System.out.println("SHORTEST PATH HAS " + str1.get(length-2)+" INTERCHANGES");
						System.out.println("~~~~~~~~~~~~~~~~~~~~~");
						System.out.print("START  ==>  " + str1.get(0) + " ==>  ");
						for(int i=1; i<length-3; i++)
						{
							System.out.println(str1.get(i));
						}
						System.out.print(str1.get(length-3) + "   ==>    END");
						System.out.println("\n~~~~~~~~~~~~~~~~~~~~");
